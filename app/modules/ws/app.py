            ...
            if message['type'] == 'message':
                try:
                    ...
                    for sid in connections:
                        socketio.emit('research_event', event_data, room=sid)
                    
                    # Stop listening if research is complete or errored
                    if event_data.get('event_type') in ['research_complete', 'research_error']:
                        print(f"Research {research_id} completed, stopping listener")
                        break
...
def run():
    app = init_socketio()
    app.run(host='0.0.0.0', port=5000, debug=True if os.getenv('ENV') == "debug" else False)